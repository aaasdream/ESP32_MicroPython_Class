...
def set_servo_angle(angle):
    """根據 0-180 度的值設定伺服馬達角度"""
    if not 0 <= angle <= 180:
        return
    # 0.5ms (0度) 到 2.5ms (180度) -> 500,000 ns 到 2,500,000 ns
    min_ns = 500000
    max_ns = 2500000
    duty_ns = int(min_ns + (angle / 180) * (max_ns - min_ns))
        ...
        while True:
            ...
            try:
                ...
                if 'led_color' in data:
                    color_hex = data['led_color']
                    r = int(color_hex[1:3], 16)
                    g = int(color_hex[3:5], 16)
                    b = int(color_hex[5:7], 16)
                    np.fill((r, g, b)) # 將兩顆 LED 設為相同顏色
                    np.write()
                # 處理伺服馬達角度指令
                elif 'servo_angle' in data:
